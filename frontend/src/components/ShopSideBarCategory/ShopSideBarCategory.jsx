          ...
          <div  className={`${Styles.normal_flex} relative overflow-hidden  px-1`}
           >
            ...
            <h3 className='ml-[3px] mr-[5px] my-3
             cursor-pointer select-none text-[#242424] font-[400] text-[13px]'
             onClick={() =>{
              handleSubmit(item);
              setShowSideBer(false);
             }}>{item.catname}</h3>
            <div className='absolute right-[2px] h-[22px] w-[22px] border-[1px] border-[rgb(36,36,36,0.4)]'
            title='Sub-categories'
            onClick={()=>{
              setShowSubCat(item);
              toggleCategory(index);
            }}>
              <FaAngleDown className={`arrowM text-[#242424] absolute right-[3px] top-[4px]`}/>
            </div>
          </div>
          <div>
                  ...
                  <div className='subCategoryItemsM'>
                    {item.sub_categories.map((subcat) => (
                      <div key={subcat.id} className='itemAndSvg flex items-center ml-[15px] gap-[15px] mb-[5px]'>
                        <FaArrowRight size={15} className='icon text-[#242424]'/>
                        <span  className='subCategory-itemM font-[400] text-[12px] text-[#242424]'
                           onClick={(e) => {
