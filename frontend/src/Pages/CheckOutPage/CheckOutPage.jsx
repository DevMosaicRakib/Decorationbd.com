...
const CheckOutPage = ({autoclose,shipping_charge,deviceCookie}) => {
    ...
    useEffect(() => {
      if (carts && carts.length > 0) {
        ...
        const totalPrice = carts?.[0]?.total_price;  // (sub_total is used temporary for checked)
    ...
    const DeliveryCharge = shipping_charge?.length ? Number(shipping_charge?.[0]?.delivery_charge) : 0
    ...
    const updateQuantity = async (itemId, newQuantity) => {
      try {
        ...
        cartItems(); // Refetch cart items after updating quantity
      } catch (error) {
        console.error('Error updating quantity:', error);
      }
    };
      ...
      shadow shadow-[#077bc4] bg-[#f7f7f7] mt-[10px] 1280px:mt-[5px] 1350px:mt-[2px] mb-[10px] 768px:mb-0">
        ...
        <div className="w-full px-2 py-2 bg-white shadow shadow-[gray] mt-[10px] 1350px:mt-0">
            ...
            {carts?.length ? (
              ...
              {carts?.map((item)=>(
                   ...
                   <img src={process.env.REACT_APP_IMG_URL+item?.product?.product_imgs[0]?.images} alt="" className='w-[40px] h-[40px] object-cover cursor-pointer 
                   768px:mx-[5px] mx-[5px]'/>
                   <div className="flex flex-col justify-center">
                   <h5 className=' font-[400] pr-[14px] 768px:pr-0 text-[10px] text-[#242424]'>{item?.product?.name.length>20?item?.product?.name.slice(0,20)+"...":item?.product?.name}</h5>
                   ...
                   <span className=' text-[12px] font-[400] 768px:pr-3 pr-1
                    text-[#077bc4]'>{Number(item?.sub_total)}<strong className='text-[13px] font-Roboto'>৳</strong></span>
                </div>
              ))}
              </div>
            ) : (
            ...
            <div className=" 1280px:mt-0 py-3 1280px:py-2 flex items-center justify-between border-b border-[rgba(0,0,0,0.2)]">
              ...
              <span className='text-[12px] font-[400] 768px:pr-3 pr-1
                text-[#077bc4]'>{Number(totalSum)}<strong className='text-[12px] font-Roboto'>৳</strong></span>
            </div>
            <div className="mt-[10px] py-3 1280px:py-2 1280px:mt-0 flex items-center justify-between border-b border-[rgba(0,0,0,0.2)]">
              <h5 className='text-[#242424] text-[12px] font-[400] capitalize 768px:pl-4 pl-1'>
                 delivery charges :</h5>
              <span className='text-[12px] font-[400] 768px:pr-3 pr-1
                text-[#077bc4]'>{DeliveryCharge}<strong className='text-[12px] font-Roboto'>৳</strong></span>
            </div>
             <div className="mt-[10px] py-3 1280px:py-2 1280px:mt-0 flex items-center justify-between border-b border-[rgba(0,0,0,0.2)]">
              ...
              <span className='text-[12px] font-normal 768px:pr-3 pr-1
                text-[#077bc4]'>{Number(totalSum+DeliveryCharge)}<strong className='text-[12px] font-Roboto'>৳</strong></span>
            </div>
              ...
              <span className='text-[12px] font-[400] 768px:pr-3 pr-1
                text-[#077bc4]'>{Number(afterConfirmationTotal)}<strong className='text-[12px] font-Roboto'>৳</strong></span>
            </div>
                  <div className="mt-[10px] py-3 1280px:py-2 1350px:mt-0 flex items-center justify-between border-b border-[rgba(0,0,0,0.2)]">
              <h5 className='text-[#242424] text-[12px] font-[400] capitalize 768px:pl-4 pl-1'>for order confirmation :</h5>
              <span className='text-[12px] font-[400] 768px:pr-3 pr-1
                text-[#f51919]'>{Number(forcodorderconfirmation)}<strong className='text-[12px] font-Roboto'>৳</strong></span>
            </div></div>
          ...
          flex flex-col items-center overflow-hidden`}>
            ...
            <button type='submit' className='w-[60%] h-[35px] 1350px:h-[30px] 1280px:w-[50%] bg-[#f85606] my-[15px] 1280px:my-[5px]
             ...
             onClick={()=>{handleSubmit(fullPayment)}}>pay now (<span className=' font-normal text-[12px]'>
              {Number(totalSum+DeliveryCharge)}<strong className='text-[12px] font-Roboto'>৳</strong></span>)</button>
          </div></div>  
            }
        <div className=" 1280px:py-0 768px:px-4 1350px:px-3 px-2">
           ...
           onClick={()=>{setPartialCod(!partialCod);setPartialPaymentButton(!partialPaymentButton);setPaymentButton(false);
            ...
            {/* <input
              type="checkbox"
              className="w-[18px] h-[18px] xl:w-[12px] xl:h-[12px] cursor-pointer mb-1 sm:mb-0 xl:mb-[15px] 
              outline-none border border-[rgba(0,0,0,0.2)] mr-[10px]"
              onClick={(e) => {
                e.preventDefault(); // Prevent checkbox from being checked
                e.target.checked = false; // Ensure it stays unchecked
                toast.error('COD system is temporarily down. It will be up and running soon.', { autoClose: 3000 });
              }}
            /> */}
        ...
        <div className={`${partialPaymentButton?"openButton2":"closeButton2"} px-2 1350px:px-2 py-1 1350px:py-[2px] 
          flex flex-col items-center `}>
            ...
            <button type='submit' className='w-[60%] h-[35px] 1350px:h-[30px] 1280px:w-[50%] bg-[#f85606] my-[15px] 1280px:my-[5px]
             text-white font-[400] rounded-sm capitalize text-[12px] text-center'
             onClick={()=>{handleSubmit(fullPayment)}}>pay now (<span className=' font-normal text-[12px]'>
              {Number(forcodorderconfirmation)}<strong className='text-[12px] font-Roboto'>৳</strong></span>)</button>
          </div> 
