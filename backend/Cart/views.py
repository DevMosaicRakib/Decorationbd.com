...
class CartViewSet(viewsets.ModelViewSet):
    ...
    def add_item(self, request, *args, **kwargs):
        ...
        if user:
            cart_item, created = Cart.objects.get_or_create(user=user, product=product, variant=variant)
        else:
            if not device:
                return Response({"error": "Device ID is required"}, status=status.HTTP_400_BAD_REQUEST)
            cart_item, created = Cart.objects.get_or_create(device=device, product=product, variant=variant)
